<template>
        <div class="rigfix">
          <div>
            ...
            <span class="pinfen">评分:  {{househscore[0]}}分</span>
          </div>
          <div class="order">
            <p class="p1" style="padding:12px;">
              <span>￥</span><span style="font-size:30px;">{{houseprice[0]}}</span></p>
            ...
            <router-link to="/write">
              <button style="width:40%;height:45px;background-color:#FF666A;border:0 solid #fff;color:#fff;display: inline-block;">立即预定</button>
            </router-link>
    ...
    export default {
      ...
      mounted(){
          ...
          axios.get(  `http://localhost:3000/house/details/`+this.hId).then(function (result) {
              _this.houseprice.push(result.data.data[0].hPrice);
